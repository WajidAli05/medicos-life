...
import { ALLOW_INDEXING, owner, WATERMARKS } from "@/lib/site";
...
export const metadata: Metadata = {
  ...
  // a demo build stays out of search results until it's the buyer's real site
  ...(ALLOW_INDEXING ? {} : { robots: { index: false, follow: false, nocache: true } }),
  ...(WATERMARKS ? { authors: [{ name: owner.name }] } : {}),
  description:
...
export default function RootLayout({
  ...
  return (
    ...
    <html lang="en" className={`${fraunces.variable} ${manrope.variable}`} suppressHydrationWarning>
      <body className={`grain${WATERMARKS ? " demo-mode" : ""}`} style={WATERMARKS ? { paddingTop: BANNER_H } : undefined} suppressHydrationWarning>
        {WATERMARKS && (
          <>
